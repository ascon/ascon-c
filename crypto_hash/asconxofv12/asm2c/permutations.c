...
#define SBOX(x0, x1, x2, x3, x4, r0, t0, t1, t2) do {\
    t1 = x0 ^ x4; \
    t2 = x3 ^ x4; \
    t0 = -1; \
    x4 = x4 ^ t0; \
...
#define SRC(o, h, l, amt) do { \
    o = (((u64)h << 32) | l) >> amt; \
} while (0)

#define LINEAR(dl, dh, sl, sh, sl0, sh0, r0, sl1, sh1, r1, t0) do {\
    SRC(dl, sh0, sl0, r0); \
    SRC(dh, sl0, sh0, r0); \
    dl = dl ^ sl; \
    dh = dh ^ sh; \
...
void P(state *p, u8 round_const, u8 rounds) {
    ...
    p->x0.h = x0h; p->x0.l = x0l;
    p->x1.h = x1h; p->x1.l = x1l;
    p->x2.h = x2h; p->x2.l = x2l;
    p->x3.h = x3h; p->x3.l = x3l;
    p->x4.h = x4h; p->x4.l = x4l;
}
